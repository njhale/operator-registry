...
func TestDerivedProperties(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{
			name: "PackageFromCSVAnnotationsHasPrecedence",
			args: args{
				csv: &ClusterServiceVersion{
					ObjectMeta: metav1.ObjectMeta{
						Annotations: map[string]string{
...
func TestJoinProperties(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			assert.ElementsMatch(t, tt.out, joinProperties(tt.in))
		})
	}
}

func TestExpandProperties(t *testing.T) {
	pkgProperty := func(pkg, version string) Property {
		return Property{
			Type: PackageType,
			Value: mustMarshal(t, PackageProperty{
				PackageName: pkg,
	...
	for _, tt := range []struct {
		...
		{
			name: "MissingPackageProperty",
			expected: expected{
				err: true,
			},
		},
		{
			name: "TooManyPackageProperties",
			args: args{
				properties: []Property{
					pkgProperty("foo", "1.0.0"),
					pkgProperty("bar", "1.0.0"),
				},
			},
			expected: expected{
				err: true,
		...
		t.Run(tt.name, func(t *testing.T) {
			in := Bundle{
				Properties: tt.args.properties,
				version:    tt.args.version,
			}

			err := expandProperties(&in)
			if tt.expected.err {
				assert.Error(t, err)
				return
			}
			...
			assert.NoError(t, err)
			assert.Equal(t, tt.expected.pkg, in.Package)
		})
	}
}
